    ...
    <div className="w-full max-w-4xl mx-auto">
      <form onSubmit={handleSubmit} className="w-full">
        <div className="relative" ref={resultsRef}>
          ...
          {/* Dropdown unificado - Resultados + Histórico */}
            ...
            <div className="absolute top-full mt-2 w-full bg-slate-800 border-2 border-cyan-500/30 rounded-xl shadow-2xl z-50 max-h-[500px] overflow-y-auto">
              <div className="p-2">
                ...
                {searchResults.length > 0 && (
                  <div className="mb-2">
                    <div className="text-xs text-slate-400 px-3 py-2 border-b border-slate-700 mb-2 flex items-center gap-2">
                ...
                {searchHistory.length > 0 && (
                  <div className={searchResults.length > 0 ? 'pt-2 border-t border-slate-700' : ''}>
                    ...
                    {searchHistory.slice(0, 5).map((item, index) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2 hover:bg-slate-700/70 rounded-lg transition-all text-left group"
                      >
                        ...
                        <span className="flex-1 text-slate-300 group-hover:text-white transition-colors">{item}</span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {isSearching && (
                  <div className="px-3 py-8 text-center text-slate-400">
                    <svg className="animate-spin h-10 w-10 mx-auto mb-3 text-cyan-400" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                    </svg>
                ...
                {!isSearching && searchError && (
                  <div className="px-3 py-8 text-center text-red-400">
                    ...
                    <p className="text-xs mt-1 text-slate-400">{searchError}</p>
                    <p className="text-xs mt-2 text-slate-500">Tente novamente em alguns segundos</p>
                  </div>
                )}

                {/* Mensagem se não houver nada */}
                {!isSearching && !searchError && searchResults.length === 0 && searchHistory.length === 0 && query.length >= 2 && (
                  <div className="px-3 py-8 text-center text-slate-400">
